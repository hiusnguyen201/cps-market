$(document).ready(function() {
    ...
    $('.attributeFields').on('click', '.removeAttributeCurrent', function() {
        const attributeId = $(this).closest('.row').find('.attributeData').data("attid");
        attributeToDelete.push(attributeId);
        $(this).closest('.attributeFields').remove();
        $("form.form-update-all").append(`<input type="hidden" name="attributes_delete" value='${JSON.stringify(attributeToDelete)}'>`);
    });
    ...
    $("form.form-update-all").submit((event) => {
        ...
        $("form.form-update-all").append(`<input type="hidden" name="attributes_update" value='${JSON.stringify(formData)}'>`);
    ...
    $('#addAttribute').click(function() {
        $('#attributeFields').append('<div class="row"><div class="col-11"> <input type="text" name="attributes[]" class="form-control mt-2" placeholder="Enter attribute..."></div><div style="margin-top: .5rem !important;" class="col-1"> <button type="button" class="btn btn-danger" id="removeAttribute"><i class="fas fa-trash-alt"></i></button></div></div>');
    });
});